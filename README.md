...
self.canvas             # Access to the canvas that can be drawn on
self.stream             # If it is a video or a webcam, it provides access to the video stream. If it is an image it gives access to the image array
self.settings           # Access to the applied settings
self.frame              # Access to the capture frame from the stream if it is a video or a webcam
self.face_locations     # Access to the locations of the faces detected
self.face_encodings     # Access to face encodings / face signature
self.face_labels        # Access to face labels
self.detections         # Access to zipped version of (face_locations, label)
self.face_landmarks     # Access to face feature landmarks
self.face_extracts      # Access to face extracted face image arrays
